...
| **光照强度** | 按法线做明暗 | **立体感主要来自这里**。0 = 平涂，看着是平的 |
| **凹陷增强** | 把褶皱、缝隙、凹坑压暗 | 越大越有体积感，过大会显脏。只有生成式模式有真实凹陷数据 |
| **高光 / 环境光** | 材质与对比 | 高光大偏塑料感；环境光小则对比强、立体感更硬 |
| **点大小 / 不透明度 / 边缘柔和** | 视觉调整 | — |
...
点云默认的画法是**平涂** —— 每个点直接画自己的颜色，没有任何明暗变化。
没有明暗，人眼就读不出表面朝向，**再准确的三维形体看起来也是平的**。
所以每份点云都带上了逐点法线，渲染时做真实光照：

- **漫反射**（朗伯，双面）—— 形体的基本明暗。点云没有正反之分，背光面翻过来照，否则整片死黑
- **高光**（Blinn-Phong）—— 给一点真实材质的反光
- **边缘光** —— 勾出轮廓，让形体从背景里跳出来
- **凹陷遮蔽（AO）** —— 把褶皱和缝隙压暗，体积感和"真实"主要来自这一层

光在**相机空间**里固定，所以转动模型时光跟着观察者走，任何角度都能看清结构。
计算放在顶点着色器：点精灵通常只有几个像素，逐片元算纯属浪费。

法线的来源分两类：

| 模式 | 法线 | 凹陷遮蔽 |
|---|---|---|
| ⚡ 浏览器 / 🎯 高精度 | 深度网格上右邻与下邻的差向量叉积 | 无 —— 浮雕没有真正的凹陷，不编造 |
| 🧊 单图 / 🎭 多视图 | 网格面法线（精确） | 有，见下 |

AO 用的是「邻域质心相对法线的偏移」：取每点最近的 24 个邻居求质心，
凸起处质心落在法线反方向，凹陷处落在正方向。比光线投射式 AO 便宜得多
（一次 kNN 查询），但要的就是「缝隙压暗」这个效果，够用。

传输上法线用 int8（各分量 ×127）：着色只要方向，0.9° 的精度足够，
比 float32 省 4 倍带宽 —— 60 万点是 1.8MB 和 7MB 的差别。
